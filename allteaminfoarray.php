...
/*----------------USED TO LOAD ALL TEAM DATA ON POOL_PICKEM DB. THIS SCRIPT IS NOT BEING USED!!------------------*/



//include 'PHPs/functions.php';
//$objectname=array("BUF","MIA","NE", "NYJ","BAL","CIN","CLE","PIT","HOU","IND","JAC","TEN","DEN","KC" ,"OAK","SD" ,"DAL","NYG","PHI","WAS","CHI","DET","GB" ,"MIN","ATL","CAR","NO" ,"TB" ,"ARI","STL","SF" ,"SEA");
//$team=array(array());
//$teamobjects=array();
$allteamsarray=array(
...
$sql = 'INSERT INTO `pool_pickem`.`allteaminfo` (`id`, `nickname`, `city`, `abbr`, `conference`, `division`, `url`, `teamPage`, `facebook`, `twitter`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
$result = $db->prepare($sql);
for($i=0; $i<32; $i++)
{
$name=$objectname[$i];	
$teamobjects[$i]=json_decode($allteamsarray[$i]);
$team[$name] = $teamobjects[$i]->{$name};
$j=$i+1;
$id= $j;//  id

  $nickname = $team[$name]->{'nickname'};
      $city = $team[$name]->{'city'};
      $abbr = $team[$name]->{'abbr'};
$conference = $team[$name]->{'conference'};
  $division = $team[$name]->{'division'};
       $url = $team[$name]->{'url'};
  $teamPage = $team[$name]->{'teamPage'};
  $facebook = $team[$name]->{'facebook'};
   $twitter = $team[$name]->{'twitter'};

$result->bind_param("isssssssss",$j,$nickname,$city,$abbr,$conference,$division,$url,$teamPage,$facebook,$twitter);
$result->execute();
}
?>